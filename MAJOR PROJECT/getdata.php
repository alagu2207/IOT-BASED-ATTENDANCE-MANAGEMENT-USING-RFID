...
if(isset($_GET['device_token']) && isset($_GET['fingerID'])){
    $device_uid = $_GET['device_token'];
    $fingerID= $_GET['fingerID'];
if ($fingerID == 1) {
    // If the fingerprint ID is 0, update the database with the new fingerprint ID
    $sqlUpdate = "UPDATE users SET fingerID=? WHERE fingerID=0";
    $resultUpdate = mysqli_stmt_init($conn);
    if (!mysqli_stmt_prepare($resultUpdate, $sqlUpdate)) {
        echo "SQL_Error_Update_Fingerprint";
        exit();
    } else {
        // Assuming $newFingerID contains the new fingerprint ID
        mysqli_stmt_bind_param($resultUpdate, "i", $fingerID);
        if (mysqli_stmt_execute($resultUpdate)) {
            echo "Fingerprint ID updated in the database.";
        } else {
            echo "SQL_Error_Update_Fingerprint";
        }
    }
